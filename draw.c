...
void add_sphere( struct matrix * edges,
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt <= longStop; longt++ ) {
      ...
      i0 = index;
      i1 = (index + 1) % points->lastcol;
      i2 = (index + steps + 1) % points->lastcol;
      if(longt != steps-2){
        add_polygon(edges,points->m[0][i0],points->m[1][i0],points->m[2][i0]
                         ,points->m[0][i1],points->m[1][i1],points->m[2][i1]
                         ,points->m[0][i2],points->m[1][i2],points->m[2][i2]);
      }
      if(longt != 0){
        add_polygon(edges,points->m[0][i0],points->m[1][i0],points->m[2][i0]
                         ,points->m[0][i2],points->m[1][i2],points->m[2][i2]
